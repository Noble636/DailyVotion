...
function ManageUser() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch('http://localhost:5000/api/admin/admins').then(res => res.json())
  ...
  const handleDeleteConfirmed = async () => {
    ...
    let url = "";
    if (deleteType === "admin") {
      url = `http://localhost:5000/api/admin/admin/${deleteId}`;
    } else if (deleteType === "user") {
      url = `http://localhost:5000/api/admin/user/${deleteId}`;
    }
    try {
